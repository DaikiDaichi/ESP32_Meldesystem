...
void setup() {
  ...
  // Radar in Engineering Mode versetzen
...
void checkNFC() {
  ...
  if (uidLength == allowedUIDLength && memcmp(uid, allowedUID, allowedUIDLength) == 0) {
    ...
    if (radarActive) {
      ...
      breathingTransition(15000);          // 15 s Atmungs‑Effekt
      setLED(255, 0, 0);
...
void loop() {
  ...
  // periodisches Scanning 5 s Pause / 3 s aktiv
  if (radarActive) {
  ...
  // alle 200 ms NFC prüfen
  if (now - lastNFC >= 200) {
    checkNFC();
    lastNFC = now;
  }

  // Radar nur, wenn aktiv und in Scan‑Phase
